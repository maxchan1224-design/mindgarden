import { db } from '../db';
import type { Entry, PersonaId, DialogueTurn } from '../domain';

export interface AiReply { text: string; safety: boolean; offerSummary?: boolean; }
...
interface RequestBody {
  ...
  ctx: { name: string; personaId: PersonaId; isFirstResponseToday: boolean; voiceMode: boolean };
  memory: string;
  payload: {
...
export async function askAi(body: RequestBody): Promise<AiReply> {
  try {
    ...
    return { text: data.text, safety: !!data.safety, offerSummary: !!data.offerSummary };
  } catch {
    return FALLBACK;
  }
}
